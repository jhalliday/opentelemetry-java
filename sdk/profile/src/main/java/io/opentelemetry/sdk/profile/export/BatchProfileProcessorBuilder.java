...
/** Builder class for {@link BatchProfileProcessor}. */
public final class BatchProfileProcessorBuilder {
  ...
  BatchProfileProcessorBuilder(ProfileExporter profileExporter) {
   ...
   */
  public BatchProfileProcessorBuilder setScheduleDelay(long delay, TimeUnit unit) {
   ...
   * DEFAULT_SCHEDULE_DELAY_MILLIS}ms.
   */
  public BatchProfileProcessorBuilder setScheduleDelay(Duration delay) {
   ...
   */
  public BatchProfileProcessorBuilder setExporterTimeout(long timeout, TimeUnit unit) {
    requireNonNull(unit, "unit");
   ...
   * defaults to {@value DEFAULT_EXPORT_TIMEOUT_MILLIS}ms.
   */
  public BatchProfileProcessorBuilder setExporterTimeout(Duration timeout) {
  ...
  /**
   * Sets the maximum number of profiles that are kept in the queue before start dropping. More
   * memory than this value may be allocated to optimize queue access.
   *
   * <p>Default value is {@code 2048}.
   *
   * @param maxQueueSize the maximum number of profiles that are kept in the queue before start
   *     dropping.
   * @return this.
   * @see BatchProfileProcessorBuilder#DEFAULT_MAX_QUEUE_SIZE
   */
  public BatchProfileProcessorBuilder setMaxQueueSize(int maxQueueSize) {
   ...
   * @return this.
   * @see BatchProfileProcessorBuilder#DEFAULT_MAX_EXPORT_BATCH_SIZE
   */
  public BatchProfileProcessorBuilder setMaxExportBatchSize(int maxExportBatchSize) {
   ...
   */
  public BatchProfileProcessorBuilder setMeterProvider(MeterProvider meterProvider) {
  ...
  /**
   * Returns a new {@link BatchProfileProcessor} that batches, then forwards them to the given
   * {@code profileExporter}.
   *
   * @return a new {@link BatchProfileProcessor}.
   */
  public BatchProfileProcessor build() {
    return new BatchProfileProcessor(
        profileExporter,
        meterProvider,
        scheduleDelayNanos,
        maxQueueSize,

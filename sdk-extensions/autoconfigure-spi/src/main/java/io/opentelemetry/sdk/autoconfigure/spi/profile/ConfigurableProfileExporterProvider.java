...
package io.opentelemetry.sdk.autoconfigure.spi.profile;
 ...
 * autoconfigured SDK. If the {@code otel.profile.exporter} property contains a value equal to what
 ...
 */
public interface ConfigurableProfileExporterProvider {

  /**
   * Returns a {@link ProfileExporter} that can be registered to OpenTelemetry by providing the
   * property value specified by {@link #getName()}.
   */
  ProfileExporter createExporter(ConfigProperties config);

  /**
   * Returns the name of this exporter, which can be specified with the {@code
   * otel.profile.exporter} property to enable it. The name returned should NOT be the same as any
